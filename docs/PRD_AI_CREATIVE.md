...
| `acts[i].narratives[j]` | → 叙事线的一个条目（叙事段落文字） |
| `acts[i].narratives[j].shots[k]` | → 视频线的一个条目（引用 segment） |
| `acts[i].narratives[j].shots[k].narration` | → 旁白线的一个条目 |
| `acts[i].narratives[j].shots[k].emotion` | → 情绪线的锚点值 |
| `acts[i].narratives[j].shots[k].use_asr && segment.asr` | → 字幕线的一个条目（填充 ASR 内容） |

### 6.2 时间轴计算

1. 三层结构：主旨（章节）→ 叙事（段落）→ 镜头（句子）
2. 从第一个 shot 开始，按顺序排列在视频线上
3. 每个 shot 的时长 = 该 segment 的 `time_end - time_start`
4. 相邻 shot 紧密排列（无间隔）
5. 叙事线条目的时间范围 = 该叙事段落内所有 shot 的时间范围之和
6. 主旨线条目的时间范围 = 该幕内所有叙事段落的时间范围之和
7. 情绪线在每个 shot 的中点位置设置锚点，值为 `emotion`
...
```python
# 三层结构：act(主旨) → narrative(叙事段落) → shot(镜头)
tracks = []
position = 0
for act in result["acts"]:
    act_start = position
    for narrative in act["narratives"]:
        nar_start = position
        ...
        # 叙事线（每个 narrative 段落一个条目，跨多个 shot）
