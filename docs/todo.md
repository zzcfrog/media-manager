...
## 已完成：视频轨道片段左对齐（2026-06-07）

视频轨道片段始终从 0 秒开始紧密排列，无间隙。每次保存前自动归一化：按 time_start 排序后，逐个片段从 0 开始顺延计算 time_start/time_end，保持源时长不变。

**改动文件：**
- `frontend/js/workbench.js` — 新增 `_normalizeVideoTrack()` 方法（按 time_start 排序 → 从 0 逐个顺延 → 保持源时长）；`_trackSave()` 在持久化前自动调用，确保任何编辑操作（删除/分割/拖放/撤销重做）后视频轨道始终左对齐

**功能说明：**
- 删除片段：后续片段自动前移填补空隙
- 拖放片段：插入后整体重新左对齐
- 撤销/重做：恢复后同样自动归一化
- 片段源时长（`time_end - time_start` 差值）保持不变，仅调整时间线位置
