...
## 已完成：高级筛选面板二十一轮——拍摄日期控件 176→200px 显完整范围（2026-08-22）

「日期空间的宽度稍微再宽一些，现在后面那个时间的月和日都没有显示出来。」（[main.css](../frontend/css/main.css)）：

1. 实测确诊：完整范围 `2024-03-15 ~ 2024-06-20` 文本 139px，176px 控件减两侧图标区（左侧日历 26px + 右侧 ×）余输入区仅 123px——**溢出 19px 尾部月日被裁**。
2. `.adv-dim .q-input` 宽 **176→200px**：输入区 147px，文本 139px 两侧各余 ~4px；值格式恒定（mask YYYY-MM-DD，两端都满 10 字符即最长），不会更长。
- **实测**：scrollWidth 147 = clientWidth 147 无溢出、× 正常显示、日期所在行（方向 108 + 日期 200）与上行三下拉对齐正常；视觉检查完整范围含尾部 06-20 可见、居中、无重叠。

## 已完成：高级筛选面板二十轮——选中态清除 × 右移距收 5px（2026-08-22）

「当被选中时，右侧的小叉有点太靠右了，再往左挪 5px；没被选中的时候的那个向下的三角不需要挪。」（[main.css](../frontend/css/main.css)）：

1. `.adv-clear-icon` `margin-right` **-2→3px**：选中态 × 右缘从 4px（与三角右缘平齐）缩进到 **9px**，贴边太挤的观感缓解；空态下拉三角不动（仍 4px）。
- **实测**：选中字段 × 右缘缩进 9.0px（恰左移 5px）、空态三角右缘 4.0px 未变、有值隐藏三角规则不回归。

## 已完成：高级筛选面板十九轮——图标贴左三角贴右 + 组段两行 grid 改 flex 相机型号归位（2026-08-22）

两项诉求（[gallery.js](../frontend/js/gallery.js) / [main.css](../frontend/css/main.css)）：

1. **图标/三角贴边**：「下拉菜单的图标应该再往左一些，展开和收起的小三角应该再往右一些。」——对称缩两侧内距：prepend 内距 8→**0**（图标落在 12px 控件内距线上，图标左缘 20→12px）+ 控件 `padding-right` 12→**4**（三角右缘 12→4px）。两侧各缩 8px、文字区不对称度不变，故有值态 `padding-left:2px` 居中对冲仍成立（实测值中心偏 1px）。
2. **空态居中基准重标定**：图标/三角贴边后原 `left:-22px; right:-30px` 外扩失衡（空态标题右偏 6px）。**负 right 值是把 label 右缘进一步推向右**——实测两组数据得线性关系 offset=(R−18)/2，标定 **`left:-14px; right:-18px`** 为零偏移（实测 0px）。
3. **组段两行 grid 改 flex**：「相机型号由于下面的拍摄日期组件比较宽，变成右对齐了。」——grid 共享列被 176px 日期撑宽（列宽 108/176/108），上行相机型号被顶右 68px。`.adv-group-dims` 改**上下两条 flex 行**（`.adv-group-row`，新 `groupRows(grp)` 方法取代 `groupColCount`：前 ⌈n/2⌉ 上行、其余下行）——全下拉恒 108px 等宽，flex 天然竖向对齐，宽日期只影响自己所在行。
- **实测**：图标左缘全 12px、三角右缘全 4px、空态标题居中偏 0px、相机型号 x 归位 1530（修前 1598）、图片 5 组全部 2 行、音乐组段 1+2 行、有值态中心偏 1px、× 右缘 4px 与三角对齐；暗亮双主题视觉四项全过（贴边协调/居中/相机与文件组紧凑无空档/整面板无错位）。
...
1. **组段竖分割线变淡**：`.adv-group` 的 `border-left` 原用 `--border`（暗 #252525 / 亮 #ddd），贴旧面板底（#141414/#fff）对比 Δ17/Δ34，贴新底只剩 Δ3/Δ17——暗色下几乎不可见。提档 `--border2`（暗 #333 / 亮 #ccc）：暗 #333 vs #222 = Δ17、亮 #ccc vs #eee = Δ34，**恰好双双还原换底前的原始对比度**（提一档 token，不造新值）。
2. **下拉整体收窄**：`.adv-dim .q-select` 120→**108px**（菜单 portal min-width 168px 不变，弹出菜单不受影响）；日期 q-input 176px 不动。
- **实测**：暗 divider rgb(51,51,51) vs panel rgb(34,34,34)、亮 rgb(204,204,204) vs rgb(238,238,238)；全部下拉宽 108px、最长占位符（相机品牌，88px 文字宽）无截断、已选值无 ellipsis；双主题视觉五项全过（分割线清晰不刺眼/无截断/间距从容/无违和）。

## 已完成：高级筛选面板十五轮打磨——按钮+面板=高级筛选整区，较工具栏反差一档零缝相连（2026-08-22）

用户进一步界定融合边界：不是「工具栏+面板」一块，而是「**高级筛选按钮（展开态）+ 面板 = 高级筛选整区**」，整区底色要**区别于筛选工具栏**（暗更亮 / 亮更深），且按钮与面板交界**不能有任何分割**（[main.css](../frontend/css/main.css)）：

1. **整区底色**：`.adv-btn-open` 底 = `.adv-filter-panel` 底 = `var(--surface2)`（亮 #eee，较工具栏 surface #fff 更深 Δ17）/ 暗 `var(--surface3)`（#222，较工具栏 #141414 更亮 Δ14）——取代十四轮「按钮透明露栏底、栏+面板同 surface」方案（两区同色即无分界，用户要的是整区从工具栏中浮出）；工具栏仍 surface，媒体卡片仍 surface，层级：页面 < 工具栏/卡片 < 高级筛选整区。
2. **按钮-面板零缝桥**：按钮向下 `box-shadow: 0 16px 0 0` 同色零模糊投影作桥——盖过按钮至面板的全部间隔（栏 8px 下内边距 + 按钮较行高的居中悬空 ~3.2px，实测总缝 11.16px；首版 10px 差 1.16px 有细缝），溢出部分被栏 overflow 裁掉不可见。
3. **栏底边展开时整条去除**（`border-bottom: 0` 取代十四轮 `transparent`——透明仍有 1px 栏底色条会隔断桥与面板；1px 高度跳变被面板开合掩盖），工具栏↔整区的边界即两者色差本身；面板底边 1px 仍作整区外缘。
- **连带**：暗色 `.sort-group` #1c1c1c→#222（十四轮提档漏了它，Δ8 防融入栏底）。
- **实测**：暗 btnBg=panelBg=rgb(34,34,34)、barBg=rgb(20,20,20) Δ14；亮 btnBg=panelBg=rgb(238,238,238)、barBg=rgb(255,255,255) Δ17；gapBarToPanel=0、栏底边 0px、桥 16px≥缝 11.16px；字段不回归（空态 Quasar 叠层 白 7%/黑 5%、active accent-dim 暗 .15/亮 .1）；双主题视觉五项全过（同色/零缝/反差方向正确/字段正常/无违和）。

## 已完成：高级筛选面板十四轮打磨——融合块（栏+面板）整块 surface 底肉眼可辨（2026-08-22）

用户澄清：十三轮理解偏差——所指并非「面板内字段与背景」而是「**整个 ribbon 区域（筛选栏+面板融合块）与页面**」的区分度太小。两步落地（[main.css](../frontend/css/main.css)）：

1. **还原十三轮两条字段规则**（空态 surface3 实色底 + 暗色选中 .26）——字段回到 Quasar filled 半透明叠层，与十二轮前完全一致。
2. **融合块整块底色**：filter-bar（原透明衬页面底）与面板（十二轮 --bg 页面底）统一改 **`var(--surface)`**——暗 #141414 / 亮 #fff，与媒体卡片**同层级同对比（Δ10）**：卡片在页面上清晰可辨，整块 ribbon 区同样肉眼可辨（十二轮 --bg 与页面零色差是「看不见」的根因）；收起时筛选栏即一条 surface 工具栏，展开/收起无底色跳变；融合连通保持（同底 + 栏底边随 `adv-open` 透明 + 按钮透明底露栏色）。
3. **连带**：暗色栏内填充控件（搜索/排序字段 #1c1c1c、媒体类型按钮组 surface2）在新栏底 #141414 上只剩 Δ8——同步提至 **#222**（Δ14 保分层）。
- **实测**：暗 barBg=panelBg=rgb(20,20,20) vs 页面 rgb(10,10,10) Δ10；亮 rgb(255,255,255) vs rgb(245,245,245) Δ10；接缝 0 + 栏底边 rgba(0,0,0,0)；逐维枚举 19 维仅「色调=暖调」active（accent-dim 亮 0.1 / 暗 0.15）、空态全部 Quasar 叠层（rgba 白 7% / 黑 5%）；暗栏内字段/按钮组均 rgb(34,34,34)；双主题视觉五项全过（整块可辨 / 连成一块无断缝 / 栏内控件清晰 / 字段与选中态正常 / 无违和）。

## 已完成：高级筛选面板十三轮打磨——字段底提级 surface3 拉开与背景对比（2026-08-22，方向理解偏差，字段改动已于十四轮还原）

用户反馈：十二轮面板底改页面底后，**字段与背景区分度太小肉眼难辨**——根因：Quasar filled 字段底是半透明叠层（暗白 7% / 亮黑 5%），在页面底上合成后暗仅 Δ17（rgb(27,27,27) vs #0a0a0a）、亮仅 Δ13（rgb(232,232,232) vs #f5f5f5）。落地（[main.css](../frontend/css/main.css) 两条规则，融合观感不变——面板底仍 = 页面底）：

1. **空态字段显式 surface3 实色底**：`.adv-filter-panel .q-field--filled .q-field__control::before` 从 Quasar 半透明叠层改为 `background: var(--surface3); opacity: 1`——暗 #222 rgb(34,34,34) **Δ24**（+41%）、亮 #dbdbdb **Δ21**（约翻倍），字段灰底块肉眼直接可辨。
2. **暗色选中态 accent .15 → .26**：`.15` 在 #0a0a0a 上合成 rgb(25,29,46)，其亮度会被空态 #222(34,34,34) 反超（选中反而不如空态醒目，亮度反差颠倒）；提到 `.26` 合成 rgb(35,44,74)，**蓝通道领先空态 +40**，选中仍最醒目。亮色选中 0.106 保持（淡蓝 tint + accent 底条/文字已足够区分）。
- **实测**：暗空态 rgb(34,34,34)/bg rgb(10,10,10) Δ24、暗选中 rgba(108,140,255,.26)、亮空态渲染 (224,224,224)/bg (245,245,245) Δ21、亮选中 (219,222,234) 蓝通道 +15；双主题视觉四项全过（字段清晰可辨 / 选中仍最醒目 / 栏-面板同底融合无缝不变 / 无突兀），选中「色调=暖调」持久化正常。
