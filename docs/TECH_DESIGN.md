...
| 高级筛选面板 | 声明式 spec `ADVANCED_FILTER_SPEC`（audio/image/video 各一组维度，image 19 / video 18 / audio 6；每 dim 可带 `group`（组 i18n 键，**同组必须连续**））+ `currentSpec` 驱动渲染；面板 **Office Ribbon 同屏组段布局**——`groupedSpec` computed 按连续 `group` 聚合成段，面板 = flex-wrap 容器，每段 `.adv-group`（段内 `.adv-group-dims` **固定两行 grid**——`display:grid` + 模板内联 `gridTemplateColumns: repeat(ceil(n/2), auto)`（`groupColCount(grp)` 方法），按行填充即上下两行 + 段顶 `.adv-group-name` 10px 组名；`flex:0 0 auto + max-width:100%` 段为固定形状块不收缩），段间 1px 竖分隔线；**组段恒单行横向滚动**——面板 `flex-wrap:nowrap + overflow-x:auto` + 6px 细滚动条，`onAdvPanelWheel`（滚轮竖转横，无溢出早退不拦截）+ `startAdvDrag`（lasso 同款 document 监听闭包拖拽滑动，4px 阈值）+ `@click.capture="onAdvPanelClickCapture"` 吞拖拽后的 click 防误开下拉（`setTimeout(0)` 复位标志）（`:first-child` 与无组段 `.adv-group-plain` 无边线、无组名）；筛选栏「高级筛选」按钮（`v-if="currentSpec.length"`，基类 `adv-toggle-btn`（`flex-shrink:0` 防栏溢出挤压折行），三态 class：展开 `adv-btn-open`（底方角）> 有活跃 `adv-btn-active` > 普通，尾部 expand_less/expand_more 箭头）+ filter-bar 根 `adv-open` 类（底边透明——按钮下侧无分隔线）；**展开态面板整块中性浅灰底（`--surface2`，暗 #1c1c1c / 亮 #eee，两主题平衡）**——十一轮弃主题色改灰（accent 只留按钮/选中态），原 `::after` accent-dim 卡舌延伸条已删（灰底上会读成蓝线）；`q-select`（dense options-dense，`:label`=维度标题；**弃 clearable**——其替换 × 偏大，改 append slot 自定义 14px `cancel` ×：有值才显示、`.adv-dim.active .q-select__dropdown-icon{display:none}` 有值隐藏三角让 × 靠右独占（`margin-right:-2px` 对齐空态三角右缘）、点击清空）全下拉——**字段定宽**：`.adv-dim .q-select` 120px（prepend 图标 22px + 四个中文字 48px + 内距/箭头）、日期 q-input 176px，值溢出 `.q-field__native` ellipsis；**prepend 维度小图标**复用 spec 既有 `icon` 字段（14px，text3 色、`.adv-dim.active` 变 accent，零新增配置）+ 音乐显示 toggle（displayOnly）；**拍摄日期合一控件**：单个只读 q-input（placeholder 居中走 `:placeholder-shown`；注意 q-input 真 input 类 = `q-field__native`，无 `q-field__input`），**prepend 维度图标（date_range）即日历入口**（QPopupProxy 锚定图标，从 append event 图标迁来，append 只留条件清除 ×）弹 QDate `range`（`advDatePick` v-model 就地状态 + `advDatePopup` v-model 控弹层，`onAdvDateRange` 仅 from+to 齐才提交并自动关），**自定义 cancel 图标清空两端**（Quasar `clearable` 对 readonly 字段不渲染 ×，这是「日期没法取消」的根因）；**两区字段**（34px）：空态 label 全宽居中（`left:-22px; right:-30px` 双向外扩——左抵消 prepend 维度图标、右抵消下拉箭头 append，因定位父级 `.q-field__control-container` 被两侧 marginal 挤窄）、float 态 label [2,14] 小标签（0-4-0 特异性压 Quasar、`transform:none`）+ 值 [14,34]（`.q-select`/`.q-input` native 规则**拆分写**，日期 input 不吃 padding-top）；`.adv-dim.active` 强调（accent-dim 底/accent 底条/文字）；`dimOptions(dim)` 统一选项来源——枚举 + **count 独立字段**（菜单徽标渲染，非 label 拼接）+ 0 置灰（`off(v)=counts?cnt(v)===0:false` 守卫——无 facetKey（显示 toggle）与 facets 未加载/失败均不禁用）+ 并入 facets 新值、音乐从 `root.musicTax` 建项（en 值 + zh 显示）、动态 dim 走 facets；菜单 `popup-content-class="adv-select-menu"`（portal 到 body，CSS 全局写；紧凑规格 28px 行高/12px 字号/3px 内边距/**min-width 168px `!important`**——Quasar 给 portal 菜单设内联 min-width = 锚点宽（定宽后仅 120px），普通规则打不赢内联样式）+ option slot（`v-bind="scope.itemProps"` 自带 disable）渲染徽标/对勾/selected 底；`_buildParams` 遍历 spec 统一发射（displayOnly 跳过、dateRange 拆 `date_from`/`date_to`），load/loadMore/selectAll 共享；切换类型 watcher 只 `_loadFacets` 不自动展开；`_advFacets` 按类型缓存，加载成功后再补拉（曾失败自动重试，避免下拉全禁用）；`advPanelOpen` 随 `_saveFilters` 持久化、picker 模式不持久化；q-date 必须 `mask="YYYY-MM-DD"`（否则字典序比较静默失效） |
